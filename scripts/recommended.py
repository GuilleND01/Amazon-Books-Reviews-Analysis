...
avg = dfRead.groupBy('title').agg({'overall': 'avg', 'reviewerID':'count'}).orderBy(col("count(reviewerID)").desc()) 
avg = avg.withColumnRenamed("avg(overall)", "rating") 
avg = avg.withColumnRenamed("count(reviewerID)", "vals")

#Ordenamos por sus rating y vals
dfFinal= avg.orderBy(col("rating").desc(), col("vals").desc()) 
